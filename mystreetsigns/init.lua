...
-- load characters map
local chars_file = io.open(minetest.get_modpath("mystreetsigns").."/characters", "r")
local charmap = {}
local max_chars = 16
...
local signs = {
    {delta = {x = 0, y = -0.35, z = 0.399}, yaw = 0},
    {delta = {x = 0.399, y = -0.35, z = 0}, yaw = math.pi / -2},
    {delta = {x = 0, y = -0.35, z = -0.399}, yaw = math.pi},
    {delta = {x = -0.399, y = -0.35, z = 0}, yaw = math.pi / 2},
}
...
local construct_sign = function(pos)
	...
	meta:set_string("infotext", pos.x.."*"..pos.z)
	meta:set_string("text", pos.x.."*"..pos.z)
end

local destruct_sign = function(pos)
    local objects = minetest.env:get_objects_inside_radius(pos, 0.5)
    for _, v in ipairs(objects) do
        if v:get_entity_name() == "mystreetsigns:text" then
            v:remove()
        end
    end
end
...
local update_sign = function(pos, fields)
	...
	local sign_info
	if minetest.env:get_node(pos).name == "mystreetsigns:sign_yard" then
	...
	local text = minetest.env:add_entity({x = pos.x + sign_info.delta.x,
										...
										z = pos.z + sign_info.delta.z}, "mystreetsigns:text")
...
minetest.register_node(":default:sign_wall", {
    ...
    on_place = function(itemstack, placer, pointed_thing)
        ...
        if wdir == 0 then
			...
			return itemstack
        elseif wdir == 1 then
            minetest.env:add_node(above, {name = "mystreetsigns:sign_yard", param2 = fdir})
        ...
        local text = minetest.env:add_entity({x = above.x + sign_info.delta.x,
                                              ...
                                              z = above.z + sign_info.delta.z}, "mystreetsigns:text")
        text:setyaw(sign_info.yaw)
...
minetest.register_node("mystreetsigns:sign_yard", {
    paramtype = "light",
	sunlight_propagates = true,
    paramtype2 = "facedir",
    drawtype = "nodebox",
...
minetest.register_node("mystreetsigns:sign_forsale_wall", {
    description = "For Sale Sign",
    inventory_image = "signs_forsale_sign.png",
    node_placement_prediction = "",
    paramtype = "light",
	 sunlight_propagates = true,
    paramtype2 = "facedir",
    ...
    node_box = {type = "fixed", fixed = {-0.45, -0.15, 0.4, 0.45, 0.45, 0.498}},
    selection_box = {type = "fixed", fixed = {-0.45, -0.15, 0.4, 0.45, 0.45, 0.498}},
    tiles = {"signs_forsale_top.png", "signs_forsale_top.png", "signs_forsale_side.png", "signs_forsale_side.png", "signs_forsale.png", "signs_forsale.png"},
    groups = sign_groups,
    ...
    on_place = function(itemstack, placer, pointed_thing)
        ...
        if wdir == 0 then
            --how would you add sign to ceiling?
            minetest.env:add_item(above, "mystreetsigns:sign_forsale_wall")
			itemstack:take_item()
			return itemstack
        elseif wdir == 1 then
            minetest.env:add_node(above, {name = "mystreetsigns:sign_forsale_yard", param2 = fdir})
            sign_info = signs_yard[fdir + 1]
        else
            minetest.env:add_node(above, {name = "mystreetsigns:sign_forsale_wall", param2 = fdir})
            sign_info = signs[fdir + 1]
        end
				itemstack:take_item()
        return itemstack
...
minetest.register_node("mystreetsigns:sign_forsale_yard", {
    paramtype = "light",
	sunlight_propagates = true,
    paramtype2 = "facedir",
    light_source = 5,
...
minetest.register_craft({
    output = "mystreetsigns:sign_forsale_wall";
    recipe = {
...
minetest.register_node("mystreetsigns:street_sign", {
	description = "Street Name Sign",
    wield_image = "streetsign2.png",
--    node_placement_prediction = "",
    paramtype = "light",
	sunlight_propagates = true,
    paramtype2 = "facedir",
	...
	node_box = {
		type = "fixed",
		fixed = {
			{-0.5, -0.125, 0.4, 0.5, 0.125, 0.5}, 
			{-0.375, -0.1875, 0.4, 0.375, 0.1875, 0.5}, 
		}
	},
    selection_box = {type = "fixed", fixed = {-0.5, -0.1875, 0.4, 0.5, 0.1875, 0.5}},
	tiles = {
		"streetsignside.png",
		"streetsignside.png",
		"streetsignside.png",
    ...
    on_place = function(itemstack, placer, pointed_thing)
        ...
        if wdir == 0 then
            --how would you add sign to ceiling?
            minetest.env:add_item(above, "mystreetsigns:street_sign")
			itemstack:take_item()
			return itemstack
        elseif wdir == 1 then
            minetest.env:add_item(above, "mystreetsigns:street_sign")
			itemstack:take_item()
			return itemstack
--            minetest.env:add_node(above, {name = "mystreetsigns:street_sign", param2 = fdir})
--            sign_info = signs[fdir + 1]
--        else
--            minetest.env:add_node(above, {name = "mystreetsigns:street_sign", param2 = fdir})
--            sign_info = signs[fdir + 1]
...
minetest.register_entity("mystreetsigns:text", {
